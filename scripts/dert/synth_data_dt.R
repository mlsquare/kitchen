...
synth_dataset <- read.csv("synth_data.csv", header = TRUE)
drops <- c("X")
synth_dataset <- synth_dataset[, !(names(synth_dataset) %in% drops)]
print(summary(synth_dataset))
...
final_paths <- data.frame(matrix(ncol = 1, nrow = nrow(synth_dataset)))
...
pdf("test_plot_synth.pdf") #Outside the loop
...
# for (iter_index in 1:nrow(iris)) {
  j = j + 1
for (iter_index in 1:500) {
  print(iter_index)
    x <- synth_dataset[iter_index, 1:6]
    ...
    vnum <- match(rownames(local_tree$splits), colnames(synth_dataset[1,]))
    ...
    path_list <- return_path(synth_dataset[iter_index,])
    # print(path_list)
    print(path_list)
...
# write.csv(label_df, file="test_adult_bin_labels_1000.csv")
    ...
    node <- as.integer(rpart.predict(local_tree, synth_dataset[iter_index,], nn = TRUE)['nn'])
...
write.csv(final_paths, file="local_synth_path.csv")
label_df <- data.frame(values(global_bins))
write.csv(label_df, file="local_synth_bins.csv")
