...
final_paths <- data.frame(matrix(ncol = 1, nrow = nsample))
...
# Secondary paths
instance_paths <- matrix(ncol = nsample, nrow = ntrees)
assoc_matrix <- matrix(ncol = nsec, nrow = ntrees)
counts <- rep(1L, ntrees)
depths <- rep(0L, ntrees)
...
for (iter_index in 1:ntrees) {
  ...
  nodes_d <- as.numeric(nnum)
  depths[iter_index] <- max(rpart:::tree.depth(nodes_d))
  
  instances <- assoc_matrix[iter_index,]
  
   for(row_iter in 1:nsec){
     path_list <- return_path(dataset[row_iter, 2:ncol(dataset)])
...
## Plot trees
# png(paste0("R/graphs/", dataset_name, "_tree_",iter_index, ".png"), width = 6000, height = 6000)
